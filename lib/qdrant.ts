...
export async function getQdrantVectorStore(
  ...
  if (GEMINI_KEYS.length === 0) {
    throw new Error(
      "No Gemini API keys found in environment variables (GEMINI_API_KEY_1 to 10)",
    );
  }
  ...
  const chunks = splitDocs
    .map((doc) => cleanText(doc.pageContent))
    .filter((c) => c.length > 5);
  ...
  console.log(
    `Starting ingestion of ${chunks.length} chunks into collection '${collectionName}'...`,
  );
  ...
  for (let i = 0; i < chunks.length; i += batchSize) {
    ...
    while (!batchSuccess && attempt < GEMINI_KEYS.length) {
      ...
      console.log(
        `Batch ${Math.floor(i / batchSize) + 1} (Attempt ${attempt + 1}): Using Key #${keyIndex + 1}...`,
      );
      ...
      try {
        ...
        const batchEmbeddings =
          await embeddingModel.embedDocuments(batchChunks);
        allEmbeddings.push(...batchEmbeddings);

        // Map to Qdrant points with unique UUIDs
        const points = batchChunks
          .map((chunk, j) => {
            const vector = batchEmbeddings[j];
            if (!vector || vector.length === 0) {
              console.warn(
                `Warning: Empty vector generated for chunk ${i + j}`,
              );
              return null;
            }
            if (vector.length !== 3072) {
              console.warn(
                `Warning: Vector dimension mismatch for chunk ${i + j}. Expected 3072, got ${vector.length}.`,
              );
        ...
        if (points.length === 0) {
          console.error(
            `No valid points to upsert for batch ${Math.floor(i / batchSize) + 1}`,
          );
          batchSuccess = true;
          continue;
        }
        ...
        try {
          ...
          console.error(
            ...
            JSON.stringify(upsertError.data || upsertError, null, 2),
          );
          throw upsertError;
        }
    ...
    if (!batchSuccess) {
      throw new Error(
        `Failed batch ${Math.floor(i / batchSize) + 1} after trying all keys.`,
      );
    }
  ...
  console.log(
    `Successfully ingested ${chunks.length} chunks into Qdrant collection '${collectionName}'`,
  );
